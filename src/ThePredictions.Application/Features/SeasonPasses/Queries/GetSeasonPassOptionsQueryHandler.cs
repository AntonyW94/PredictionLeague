    ...
    public async Task<SeasonPassOptionsDto?> Handle(GetSeasonPassOptionsQuery request, CancellationToken cancellationToken)
    {
        const string sql = @"
            SELECT
                ...
                CASE WHEN s.[PassStandardPrice] IS NOT NULL THEN 1 ELSE 0 END AS RequiresPayment,
                s.[PassStandardPrice] AS StandardPrice,
                s.[PassPremiumPrice] AS PremiumPrice,
                CASE WHEN (SELECT COUNT(*) FROM [SeasonPasses] WHERE [UserId] = @UserId) = 0 THEN 1 ELSE 0 END AS IsTrialEligible,
                CASE WHEN EXISTS (SELECT 1 FROM [SeasonPasses] sp WHERE sp.[UserId] = @UserId AND sp.[SeasonId] = s.[Id]) THEN 1 ELSE 0 END AS AlreadyHeld
            FROM
                [Seasons] s
            WHERE
                s.[Id] = @SeasonId;";
