...
14. **NEVER pass Dapper a collection as a command's parameter** - it executes the statement once per element, sequentially, which is N round trips inside the transaction holding the write locks. Send the rows as one JSON parameter with `JsonRows.From(...)` and read them back with `OPENJSON(@Rows) WITH (...)`, every path in `strict` mode. See [Set-based writes](docs/guides/database.md#set-based-writes) and [ADR-0020](docs/decisions/0020-set-based-writes.md).
