...
The Domain, Validators, Contracts, Hosting.Shared and API projects **must maintain 100% line and branch coverage** - all five are gated in CI, so a drop fails the build. Infrastructure, Web.Client and Application target 100% but are measured only; see [`docs/guides/testing.md`](docs/guides/testing.md#enforcement-is-per-project-and-rolls-out-gradually) for how a project joins the gate. After writing or modifying code:
