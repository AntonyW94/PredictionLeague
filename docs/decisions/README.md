...
| [0016](./0016-business-rule-exception-classification.md) | Business rules get their own exception type | Accepted | technical | Business rules throw `BusinessRuleViolationException` (400 + Warning); `InvalidOperationException` is no longer caught and means a server fault (500 + Error). Inverts a fail-open default that hid a broken leaderboard and 18 config faults in the client-error bucket. |
