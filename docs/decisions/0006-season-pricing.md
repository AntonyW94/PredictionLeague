...
Each pass-required season stores admin-set **`EntryPrice`** and **`SmsPrice`** (DB-backed, editable in admin). Stripe Checkout uses **dynamic `price_data`** read from the DB at session creation — **no pre-created Stripe Price objects**. Validation: `RequiresPass ⇒ prices set`; `SmsPrice ≥ EntryPrice`.
