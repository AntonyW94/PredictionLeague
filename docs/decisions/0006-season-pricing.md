...
Each pass-required season stores admin-set **`PassEntryPrice`** and **`PassSmsPrice`** (DB-backed, editable in admin). Stripe Checkout uses **dynamic `price_data`** read from the DB at session creation — **no pre-created Stripe Price objects**. Validation: a season is either free (both prices null) or paid (both prices set, `PassEntryPrice > 0`, `PassSmsPrice ≥ PassEntryPrice`). `RequiresPass` is **derived** from price presence, not a stored flag.
