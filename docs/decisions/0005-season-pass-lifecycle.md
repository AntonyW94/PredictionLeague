...
A season is pass-required when it has prices. `Season` exposes a **computed** `RequiresPass => PassStandardPrice.HasValue` (no stored flag/column - refined during implementation; the original draft proposed a stored `RequiresPass` boolean, but it was redundant with `PassStandardPrice` presence).
