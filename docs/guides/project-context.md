...
The two **database** workflows use `tools/ThePredictions.DatabaseTools/`. The **dev refresh** reads all tables from production, anonymises personal data (realistic fake names/emails via Bogus), creates test accounts, and writes to the dev database. Three accounts are created, all sharing the `TEST_ACCOUNT_PASSWORD`: `testplayer@dev.local` and `testadmin@dev.local` each get membership of the first league **and a free Season Pass for that league's season**, and `testnewplayer@dev.local` deliberately gets neither. The pass is not decoration - getting a pass is a *required* onboarding step, so an account without one is stuck on the getting-started checklist and cannot reach a league or a leaderboard at all, which is why the third account exists as the way to see that state on purpose. The **production backup** copies all data unmodified to `ThePredictionsBackup` as a safety net independent of Fasthosts' own backup policy. Token tables (`AspNetUserTokens`, `RefreshTokens`, `PasswordResetTokens`) are excluded from both. The dev refresh additionally leaves `EmailSettings` untouched so the dev master email switch survives a refresh (see [Dev email gate](#dev-email-gate)).
