...
| 17 | E2E tests with Playwright | High | **Stage 1 shipped, August 2026.** `tests/E2E/ThePredictions.Web.Tests.E2E` drives five browser journeys against deployed dev (player dashboard, opening a league and rendering a leaderboard, the new-sign-up onboarding takeover, an admin screen, log out), on xunit.v3 + `Microsoft.Playwright`, run by `e2e-dev.yml` after every dev deploy and reported to `#github`. Structural assertions only, because dev is an anonymised production copy. It found that the dev test accounts had **no Season Pass**, so every one of them sat in the onboarding takeover and the dashboard never rendered a league or a leaderboard - `TestAccountCreator` now grants one, plus a third pass-less account that keeps the onboarding state on purpose. **Residual is Stage 2**, the larger half: a SQL Server container, DbUp-built schema and a seeder, which are prerequisites for E2E-ing any flow needing controlled round state (submitting a prediction, entering results, prize payouts) and for gating the deploy rather than reporting after it | [Stage 2 plan](architecture/e2e-testing/README.md) |
