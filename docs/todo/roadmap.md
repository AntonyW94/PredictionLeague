...
| 3 | Assertion library: FluentAssertions to AwesomeAssertions | Low | **Started, August 2026: `ThePredictions.Web.Tests.E2E` is on AwesomeAssertions 9.5.0 (Apache-2.0) and is the reference for the rest.** Pinned elsewhere to FluentAssertions **7.0.0**, the last free version - 8.x went commercial, so that dependency will never receive another update or security fix. Three things the first migration established: it is a genuine **namespace** change (`using AwesomeAssertions;`) and not a silent drop-in, so the compiler lists every file to touch; the failure-message wording is byte-identical, so tests asserting on `because` reasons need no rework; and **`ci.yml`'s coverage `Exclude` list names `[FluentAssertions]*`, so `[AwesomeAssertions]*` has to be added as projects move** or coverlet starts instrumenting the assertion library and the 100% figures shift for no real reason. The 13 remaining test projects are one package swap and one using each | Partly done |
...
| 17 | E2E tests with Playwright | High | **Moved up (2026-07-30)**, and **re-planned as a single stage (2026-08-14)**. The remaining backlog is mostly UX journeys, so building the harness first means each feature adds its own test rather than accruing retrospective debt - and a leaderboard broke in production on 30 July with 1,647 green unit tests. The original Stage 1 (a smoke suite against deployed dev, needing no seeder) was **built, proved green in CI, and then abandoned**: dev is a working environment whose data is changed constantly while new work is tested and periodically replaced wholesale by a refresh, so a suite asserting on it aims at a moving target and goes red for reasons that are not bugs - and a suite that cries wolf is worse than none. The isolated stack has to be built anyway to reach the flows that matter, so a second, weaker suite is not worth the upkeep. (An earlier version of this row blamed an account collision between the suite and manual testing; that was wrong, `DataAnonymiser.PreservedEmails` keeps the owner's own login working, and the plan records the correction.) Two useful things came out of it and are kept: the dev test accounts had **no Season Pass**, and `get-pass` is a required onboarding step, so every one of them sat in the onboarding takeover with no league or leaderboard reachable (`TestAccountCreator` now grants a pass, plus a third pass-less account); and the seeder is far cheaper than the plan assumed, since `ITestDataSeeder` already has 33 methods including season passes and the rank cache. **The isolated stack then shipped and works (August 2026):** `tests/E2E/ThePredictions.Web.Tests.E2E` builds a SQL Server container, applies the committed DbUp migrations, seeds a user, launches the *published* web application against it and drives real Chromium - one journey so far, a player signing in and reaching an authenticated dashboard, because the stack is the expensive part and nothing about it was proven until one journey drove it end to end. Gates pull requests, never touches dev or production. Journeys are labelled `Smoke`/`Core`/`Extended` and selectable per run via `workflow_dispatch` tickboxes; everything on an 03:00 schedule. Elements are addressed only by `data-test-id`, with a convention test reading the markup to prove each referenced id exists. **What remains is journeys, and two decisions the second one forces:** test isolation once a journey writes rather than reads, and `SkiaSharp` having no Linux native assets (harmless until a journey touches the badge PNG endpoint or a share card) | [Detailed plan](architecture/e2e-testing/README.md) |
