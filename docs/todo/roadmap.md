...
| Reminder email urgency params | The reminder handler now sends `PREDICTIONS_URL`, `URGENCY` and `TIME_REMAINING` (`ReminderUrgencyFormatter`). Plan trimmed to the remaining (manual) Brevo template redesign + test. |
