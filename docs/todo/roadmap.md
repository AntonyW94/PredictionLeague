...
|---|------|--------|-----|------|
| 1 | Machine-enforced conventions | Low-Medium | Every rule in `CLAUDE.md` is prose that nothing checks, and two have already rotted silently: ~20 `Get*Query` types return collections despite the documented Get/Fetch split, and eight reads use `SELECT *` against the SQL conventions (three in the Identity user store, two in the role store, two in the season repository, one in the refresh-token repository). `SELECT *` is the more dangerous of the two - adding a column changes what the query hands the result type's constructor, which is precisely the runtime break ADR-0016 was written about. Add a banned-symbol analyser (`DateTime.Now`, `DateTime.Today`) plus a conventions test project gating the rules that matter most - no repositories in query handlers, no `IMediator` in handlers, Get/Fetch naming, no `SELECT *` - each with an allowlist of current offenders to burn down | Idea |
