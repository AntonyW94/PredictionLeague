...
- **Payouts (peer-to-peer)** → players may **optionally** store **encrypted payout details** (visible only to the named admin(s) of leagues they're in; deletable; manual fallback if not provided; a **join-time warning** names the admin who'll gain visibility, with a remove button). At league end the admin gets a **payouts list** showing **one aggregated total per winner** (summing all their prizes in that league, tracked in `LeaguePayouts`) and **marks each total as paid** manually — a clean seam to automate later only with legal sign-off (ADR 0010, 0008; Task 20).
