...
- **Payouts (peer-to-peer)** → players may **optionally** store **encrypted payout details** (visible only to the named admin(s) of leagues they're in; deletable; manual fallback if not provided). At league end the admin gets a **payouts list** and **marks each as paid** manually — a clean seam to automate later only with legal sign-off (ADR 0010, 0008; Task 20).
