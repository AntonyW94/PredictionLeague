...
4. **Expected players = break-even denominator:** the **distinct approved participant count of the last completed season with the same `CompetitionId`** (the `Competitions` reference table — ADR 0017, not `ApiLeagueId`). `perPlayer = target / expectedPlayers`.
...
- "Comparable season" = same competition, matched on `Season.CompetitionId` (the `Competitions` table, ADR 0017).
