...
4. **Expected players = break-even denominator:** the **distinct approved participant count of the last completed season of the same competition** (same `ApiLeagueId` if set, else same `CompetitionType`). `perPlayer = target / expectedPlayers`.
