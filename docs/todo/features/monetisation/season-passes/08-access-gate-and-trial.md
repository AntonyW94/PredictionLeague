...
allowed if:
  ...
  free season (PassEntryPrice IS NULL)              -> create £0 Free pass (burns the freebie) + allow
  else (paid season):
...
- `SeasonAccessService.EnsureCanParticipateAsync(userId, seasonId)`:
  ...
  - If the season is **free** (`Season.RequiresPass` is false, i.e. `PassEntryPrice IS NULL`), **create a £0 `Free` pass** (`SeasonPass.CreateFree`) and return — this records participation so a free season **burns the freebie**.
