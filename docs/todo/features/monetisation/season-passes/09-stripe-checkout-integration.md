...
- **No Price-ID map** — amounts come from the season's admin-set `PassEntryPrice` / `PassSmsPrice` (Tasks 06–07, 15), passed to Stripe as dynamic `price_data`.

### Step 2: Create Checkout session

- `StripePaymentService.CreateCheckoutSessionAsync(userId, seasonId, tier)`:
  - Look up the season's price for the chosen tier (`PassEntryPrice` for Entry, `PassSmsPrice` for Entry + SMS). If the user is **reward-eligible** (Task 13), the SMS tier is offered at `PassEntryPrice` (uplift comped).
