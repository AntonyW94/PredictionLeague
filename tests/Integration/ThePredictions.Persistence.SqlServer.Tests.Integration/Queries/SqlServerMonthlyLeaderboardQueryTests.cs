...
/// <summary>Runs <see cref="MonthlyLeaderboardQueryConformanceTests"/> against SQL Server.</summary>
[Collection(DatabaseCollection.Name)]
[Trait(IntegrationTrait.Name, IntegrationTrait.Value)]
public class SqlServerMonthlyLeaderboardQueryTests(SqlServerDatabaseFixture fixture)
    : MonthlyLeaderboardQueryConformanceTests, IAsyncLifetime
{
    private readonly SqlServerTestHarness _harness = new(fixture);

    protected override IMonthlyLeaderboardQuery Query => new MonthlyLeaderboardQuery(_harness.ReadDbConnection);
