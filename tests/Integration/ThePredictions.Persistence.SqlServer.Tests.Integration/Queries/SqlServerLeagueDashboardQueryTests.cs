...
/// <summary>Runs <see cref="LeagueDashboardQueryConformanceTests"/> against SQL Server.</summary>
[Collection(DatabaseCollection.Name)]
[Trait(IntegrationTrait.Name, IntegrationTrait.Value)]
public class SqlServerLeagueDashboardQueryTests(SqlServerDatabaseFixture fixture)
    : LeagueDashboardQueryConformanceTests, IAsyncLifetime
{
    private readonly SqlServerTestHarness _harness = new(fixture);

    protected override ILeagueDashboardQuery Query => new LeagueDashboardQuery(_harness.ReadDbConnection);
