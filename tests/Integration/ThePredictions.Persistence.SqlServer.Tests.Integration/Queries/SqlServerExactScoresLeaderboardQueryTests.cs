...
/// <summary>Runs <see cref="ExactScoresLeaderboardQueryConformanceTests"/> against SQL Server.</summary>
[Collection(DatabaseCollection.Name)]
[Trait(IntegrationTrait.Name, IntegrationTrait.Value)]
public class SqlServerExactScoresLeaderboardQueryTests(SqlServerDatabaseFixture fixture)
    : ExactScoresLeaderboardQueryConformanceTests, IAsyncLifetime
{
    private readonly SqlServerTestHarness _harness = new(fixture);

    protected override IExactScoresLeaderboardQuery Query =>
        new ExactScoresLeaderboardQuery(_harness.ReadDbConnection);
