...
/// <summary>Runs <see cref="LeaguePaymentInfoQueryConformanceTests"/> against SQL Server.</summary>
[Collection(DatabaseCollection.Name)]
[Trait(IntegrationTrait.Name, IntegrationTrait.Value)]
public class SqlServerLeaguePaymentInfoQueryTests(SqlServerDatabaseFixture fixture)
    : LeaguePaymentInfoQueryConformanceTests, IAsyncLifetime
{
    private readonly SqlServerTestHarness _harness = new(fixture);

    protected override ILeaguePaymentInfoQuery Query => new LeaguePaymentInfoQuery(_harness.ReadDbConnection);
