...
/// <summary>Runs <see cref="StageLeaderboardQueryConformanceTests"/> against SQL Server.</summary>
[Collection(DatabaseCollection.Name)]
[Trait(IntegrationTrait.Name, IntegrationTrait.Value)]
public class SqlServerStageLeaderboardQueryTests(SqlServerDatabaseFixture fixture)
    : StageLeaderboardQueryConformanceTests, IAsyncLifetime
{
    private readonly SqlServerTestHarness _harness = new(fixture);

    protected override IStageLeaderboardQuery Query => new StageLeaderboardQuery(_harness.ReadDbConnection);
