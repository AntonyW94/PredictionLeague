...
/// <summary>
/// The hinge of the whole scoring pipeline: entering match results moves the round through its statuses,
/// scores everyone's predictions, applies boosts and refreshes the standings. Settling the round - prizes,
/// badges, emails - is CompleteRoundCommand's job, sent after this command's transaction commits, and this
/// command's only part in it is reporting that the round finished.
/// </summary>
public class ScoreMatchResultsCommandHandlerTests
{
    ...
    private readonly ScoreMatchResultsCommandHandler _handler;

    public ScoreMatchResultsCommandHandlerTests()
    {
        _handler = new ScoreMatchResultsCommandHandler(
            _boostService, _roundResultsService, _roundRepository, _predictionRepository,
            _leagueStatsRepository, _currentUserService, new TestDateTimeProvider(FixedNow));
    ...
    private static ScoreMatchResultsCommand Command(params MatchResultDto[] results) =>
        new(RoundId, results.Length == 0 ? [new MatchResultDto(MatchId, 2, 1, MatchStatus.Completed)] : results.ToList());

    private Task<MatchResultsOutcome> HandleAsync(ScoreMatchResultsCommand? command = null) =>
    ...
    public async Task Handle_ShouldNotCompleteTheRound_WhileAMatchIsStillToPlay()
    {
        ...
        var outcome = await HandleAsync(Command(new MatchResultDto(MatchId, 1, 0, MatchStatus.Completed)));

        round.Status.Should().Be(RoundStatus.InProgress);
        outcome.RoundFinished.Should().BeFalse();
    }
    ...
    public async Task Handle_ShouldCompleteTheRound_WhenEveryMatchIsDone()
    {
        ...
        outcome.RoundFinished.Should().BeTrue();
    }
    ...
    public async Task Handle_ShouldCompleteTheRound_WhenTheLastMatchIsPostponedRatherThanPlayed()
    {
        ...
        var outcome = await HandleAsync(Command(new MatchResultDto(MatchId, 0, 0, MatchStatus.Postponed)));
